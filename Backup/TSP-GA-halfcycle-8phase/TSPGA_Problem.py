...
class MyProblem(ea.Problem):
    def __init__(self):
        ...
        lb = [6, 12, 6, 12, 6, 12, 6, 12]  # 决策变量下边界
        ub = [30, 35, 20, 35, 30, 35, 20, 35]  # 决策变量上边界
...
def totperdelay(Vars):
    ...
    for j in range(Vars.shape[0]):  # the range function checks the variable passed into it and returns a series of
        ...
        ge5 = cycle + g5
        ge6 = cycle + g5 + g6 + 5
        ge7 = cycle + g5 + g6 + g7 + 10
        ge8 = cycle + g5 + g6 + g7 + g8 + 15
                ...
                if vehsp < lowspd:  # stopped vehicles
                    ...
                    if 'NE' in vehrou:
                        if time2stp <= ge1 - gs1:  # can leave at current phase
                            ...
                            delay = stptime + gs5 + time2stp - g1 - dist2stp / ns_freespd
                    ...
                    # north & south through, phase "g2"
                        ...
                        if time2stp <= ge2 - gs2:  # can leave at current phase
                            ...
                            delay = stptime + gs6 + time2stp - g2 - dist2stp / ns_freespd
                    ...
                    # east & west left turn, phase "g3"
                        ...
                        if time2stp <= ge3 - gs3:  # can leave at current phase
                            delay = max(0, stptime + gs3 + time2stp - dist2stp / ew_freespd) * g3factor
                        else:  # cannot leave at current phase
                            delay = (stptime + gs7 + time2stp - g3 - dist2stp / ew_freespd) * g3factor
                    ...
                    # east & west through, phase "g4"
                        ...
                        if time2stp <= ge4 - gs4:  # can leave at current phase
                            delay = max(0, stptime + gs4 + time2stp - dist2stp / ew_freespd) * g4factor
                        else:  # cannot leave at current phase
                            delay = (stptime + gs8 + time2stp - g4 - dist2stp / ew_freespd) * g4factor
                    ...
                    if 'NE' in vehrou:
                        if gs1 + g1nemaxqueuedelay < ge1:  # can clear the queue
                            if time2stp < gs1 + g1nemaxqueuedelay:
                                ...
                                delay = 0
                            elif ge1 < time2stp < gs5:  # arrives before next green phase
                                delay = gs5 - time2stp
                            elif gs5 <= time2stp <= ge5:
                                delay = 0
                            else:
                                delay = gs5 + cycle2 - time2stp
                        else:  # can not clear the queue
                            delay = gs5 + g1nemaxqueuedelay - g1 - time2stp
                    elif 'SW' in vehrou:
                        if gs1 + g1swmaxqueuedelay < ge1:  # can clear the queue
                            ...
                            delay = gs5 + g1swmaxqueuedelay - g1 - time2stp
                    ...
                    # north & south through, phase "g2"
                        ...
                        if gs2 + g2nsmaxqueuedelay < ge2:  # can clear the queue
                            if time2stp < gs2 + g2nsmaxqueuedelay:  # arrives before phase 2
                                ...
                                delay = 0
                            elif ge2 < time2stp < gs6:
                                delay = gs6 - time2stp
                            elif gs6 <= time2stp <= ge6:
                                delay = 0
                            else:  # arrives after phase 2
                                delay = gs6 + cycle2 - time2stp
                        else:
                            delay = gs6 + g2nsmaxqueuedelay - g2 - time2stp
                    elif 'SN' in vehrou:
                        if gs2 + g2snmaxqueuedelay < ge2:  # can clear the queue
                            ...
                            delay = gs6 + g2snmaxqueuedelay - g2 - time2stp

                    # east & west left turn, phase "g3"
                    elif 'ES' in vehrou:
                        if gs3 + g3esmaxqueuedelay < ge3:  # can clear the queue
                            if time2stp < gs3 + g3esmaxqueuedelay:  # arrives before phase 2
                                delay = (gs3 + g3esmaxqueuedelay - time2stp) * g3factor
                            elif gs3 + g3esmaxqueuedelay <= time2stp <= ge3:  # arrives at green time of phase 2
                                delay = 0
                            elif ge3 < time2stp < gs7:
                                delay = (gs7 - time2stp) * g3factor
                            elif gs7 <= time2stp <= ge7:
                                delay = 0
                            else:  # arrives after phase 3
                                delay = (gs7 + cycle2 - time2stp) * g3factor
                        else:
                            delay = (gs7 + g3esmaxqueuedelay - g3 - time2stp) * g3factor
                    elif 'WN' in vehrou:
                        if gs3 + g3wnmaxqueuedelay < ge3:  # can clear the queue
                            if time2stp < gs3 + g3wnmaxqueuedelay:  # arrives before phase 2
                                delay = (gs3 + g3wnmaxqueuedelay - time2stp) * g3factor
                            elif gs3 + g3wnmaxqueuedelay <= time2stp <= ge3:  # arrives at green time of phase 2
                                delay = 0
                            elif ge3 < time2stp < gs7:
                                delay = (gs7 - time2stp) * g3factor
                            elif gs7 <= time2stp <= ge7:
                            ...
                            delay = (gs7 + g3wnmaxqueuedelay - g3 - time2stp) * g3factor

                    # east & west through, phase "g4"
                    elif 'EW' in vehrou:
                        if gs4 + g4ewmaxqueuedelay < ge4:  # can clear the queue
                            if time2stp < gs4 + g4ewmaxqueuedelay:  # arrives before phase 2
                                delay = (gs4 + g4ewmaxqueuedelay - time2stp) * g4factor
                            elif gs4 + g4ewmaxqueuedelay <= time2stp <= ge4:  # arrives at green time of phase 2
                                delay = 0
                            elif ge4 < time2stp < gs8:
                                delay = (gs8 - time2stp) * g4factor
                            elif gs8 <= time2stp <= ge8:
                                delay = 0
                            else:  # arrives after phase 4
                                delay = (gs8 + cycle2 - time2stp) * g4factor
                        else:
                            delay = (gs8 + g4ewmaxqueuedelay - g4 - time2stp) * g4factor
                    elif 'WE' in vehrou:
                        if gs4 + g4wemaxqueuedelay < ge4:  # can clear the queue
                            if time2stp < gs4 + g4wemaxqueuedelay:  # arrives before phase 2
                                delay = (gs4 + g4wemaxqueuedelay - time2stp) * g4factor
                            elif gs4 + g4wemaxqueuedelay <= time2stp <= ge4:  # arrives at green time of phase 2
                                delay = 0
                            elif ge4 < time2stp < gs8:
                                delay = (gs8 - time2stp) * g4factor
                            elif gs8 <= time2stp <= ge8:
                            ...
                            delay = (gs8 + g4wemaxqueuedelay - g4 - time2stp) * g4factor
